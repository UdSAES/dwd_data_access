...
async function scanForNewestCosmeDe (gribBasePath, poi, forecastLength) {
  log.debug('updating cached result for /poi_forecasts/cosmo_de_%d/%s', forecastLength, poi.id)
  assert(!_.isNil(poi))
  const lon = poi.lon
  const lat = poi.lat
  const id = poi.id
  ...
  for (var i = 0; i < 8; i++) {
    ...
    _.forEach(forecast.forecasts, (item) => {
      ...
      if (item.data.length < numberOfForecastItems) {
        faulty = new Error(util.format('%s | item.data.length < numberOfForecastItems | %d < %d', item.label, item.data.length, numberOfForecastItems))
        return false
      }
...
async function run (cycleWaitTime, cosmoDeGribBasePath, poiForecastBasePath) {
  ...
  for (;;) {
    ...
    for (let i = 0; i < pois.length; i++) {
      ...
      try {
        const forecast27h = await scanForNewestCosmeDe(cosmoDeGribBasePath, poi, 27)
      ...
      try {
        const forecast45h = await scanForNewestCosmeDe(cosmoDeGribBasePath, poi, 45)
        await fs.ensureDir(path.join(poiForecastBasePath, poi.id))
    ...
    log.debug('waiting for ' + cycleWaitTime + ' seconds before next run of caching routine for POIs')
