...
const POIS_CONFIG_PATH = './configuration/pois.json'


async function scanForNewestCosmeDe(gribBasePath, poi, forecastLength) {
  console.log('new run of scanForNewestCosmeDe %s %d', poi.id, forecastLength)
  assert(!_.isNil(poi))
  const lon = poi.lon
  const lat = poi.lat
  const id = poi.id
  ...
  for (var i = 0; i < 8; i++) {
    ...
    var directResults = await Parallel.map(voisConfig, async function (item) {
      if (item.isCustomCalculation === true) {
        return null
      }

      try {
        var ts = await grib2.readTimeSeriesFromGribFiles(now.unix(), {lon: lon, lat: lat}, item.gribLabel, gribBasePath)
        if (item.label === 'down_solar_direct_radiation' || item.label === 'down_solar_diffuse_radiation') {
          var ts_new = _.cloneDeep(ts)
          const t0 = ts[0].forecastTimestamp
          for (var j = 1; j < ts.length; j++) {
            const dt = (ts[j].forecastTimestamp - ts[j-1].forecastTimestamp) / 1000
            ts_new[j].value = ((ts[j].forecastTimestamp - t0) / 1000 * ts[j].value - (ts[j-1].forecastTimestamp - t0) / 1000 * ts[j-1].value) / dt
      ...
      if (item.label === 'wind_speed_10m_ag') {
        ...
        for (var i = 0; i < zonal_wind_10m_ag.ts.length; i++) {
          ...
          const speed = Math.sqrt(
            ...
            meridional_wind_10m_ag.ts[i].value * meridional_wind_10m_ag.ts[i].value
          )
          ts.push({
        ...
        const meridional_wind_10m_ag = _.find(directResults, (item) => {return item.resultLabel === 'meridional_wind_10m_ag'})
        ...
        for (var i = 0; i < zonal_wind_10m_ag.ts.length; i++) {
          ...
          var alpha = null
          if (zonal_wind_10m_agItem.value === 0 && meridional_wind_10m_agItem.value <= 0) {
            alpha = 0
          } else if (zonal_wind_10m_agItem.value === 0 && meridional_wind_10m_agItem.value > 0) {
            alpha = 180
          } else if (meridional_wind_10m_agItem.value === 0 && zonal_wind_10m_agItem.value > 0) {
            alpha = 270
          } else if (meridional_wind_10m_agItem.value === 0 && zonal_wind_10m_agItem.value < 0) {
            alpha = 90
          } else {
            if (zonal_wind_10m_agItem.value > 0) {
              alpha = Math.atan2(zonal_wind_10m_agItem.value, meridional_wind_10m_agItem.value) * 180 / Math.PI
            } else {
              alpha = Math.atan2(zonal_wind_10m_agItem.value, meridional_wind_10m_agItem.value) * 180 / Math.PI + 360
            }
            alpha += 180
    ...
    _.forEach(results, (result) => {
      ...
      const numberOfForecastItems = forecastLength === 27 ? configItem['27hForecastLength'] : configItem['45hForecastLength']
      ...
      _.forEach(result.ts, (item) => {
        if (fc.timeseries.length >= numberOfForecastItems) {
          return
        }
    ...
    _.forEach(forecast.forecasts, (item) => {
      ...
      const numberOfForecastItems = forecastLength === 27 ? configItem['27hForecastLength'] : configItem['45hForecastLength']
      const forecastDt = forecastLength === 27 ? configItem['27hForecastdt'] : configItem['45hForecastdt']
      // check if timeseries is too short
      if (item.timeseries.length < numberOfForecastItems) {
        faulty = new Error(util.format('%s | item.timeseries.length < numberOfForecastItems | %d < %d', item.label, item.timeseries.length, numberOfForecastItems))
        return false
      }
...
async function run(cycleWaitTime, cosmoeDeGribBasePath, poiForecastBasePath) {
  ...
  for (;;) {
    ...
    for (let i = 0; i < pois.length; i++) {
      ...
      try {
        let forecast27h = await scanForNewestCosmeDe(cosmoeDeGribBasePath, poi, 27)
        await fs.ensureDir(path.join(poiForecastBasePath, poi.id))
        await fs.writeFile(path.join(poiForecastBasePath, poi.id, '27h_forecast.json'), JSON.stringify(forecast27h), {encoding: 'utf8'})
      } catch (error) {
        console.log(error)
      }

      try {
        let forecast45h = await scanForNewestCosmeDe(cosmoeDeGribBasePath, poi, 45)
        await fs.ensureDir(path.join(poiForecastBasePath, poi.id))
        await fs.writeFile(path.join(poiForecastBasePath, poi.id, '45h_forecast.json'), JSON.stringify(forecast45h), {encoding: 'utf8'})
      } catch (error) {
        console.log(error)
      }
    }
...
//run(60, '/home/dash2/florian/tmp/cosmo/de/grib', '/tmp/poi_forecasts')
