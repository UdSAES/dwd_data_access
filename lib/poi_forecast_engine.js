...
async function scanForNewestCosmeDe (gribBasePath, poi, forecastLength) {
  ...
  let voisConfig = await fs.readJson(VOIS_CONFIG_PATH)
  ...
  for (let i = 0; i < 8; i++) {
    ...
    var directResults = await Parallel.map(
      ...
      async function (item) {
        ...
        try {
          let ts = await grib2.readTimeSeriesFromGribFiles(
            now.unix(),
            { lon: lon, lat: lat },
            item.gribLabel,
            gribBasePath
            ...
            for (let j = 1; j < ts.length; j++) {
              ...
              tsNew[j].value =
                (((ts[j].forecastTimestamp - t0) / 1000) * ts[j].value -
                  ((ts[j - 1].forecastTimestamp - t0) / 1000) * ts[j - 1].value) /
                dt
            }
            ts = tsNew
          }
    ...
    const indirectResults = _.map(voisConfig, function (item) {
      if (item.isCustomCalculation !== true) {
        return null
      }
    ...
    const results = _.concat(directResults, indirectResults)
