  ...
  async function loadGribTimeseriesData(options) {
    ...
    const directoryPath = grib.deriveGrib2DirectoryPath(gribBaseDirectory, referenceTimestamp, voi)
    const timeseriesData = await grib.extractTimeseriesDataFromGrib2Directory(directoryPath, location)
  ...
  async function loadRadiationValue(options) {
    ...
    for (var j = 1; j < timeseriesData.length; j++) {
      ...
      finalTimeseriesData[j].value = ((timeseriesData[j].timestamp - t0) / 1000 * timeseriesData[j].value - (timeseriesData[j - 1].timestamp - t0) / 1000 * timeseriesData[j - 1].value) / dt
    }
