...
function getStringsFromStationDataPayloadMosmix (timeseriesDataArray) {
...
function getStringsFromStationDataPayloadCosmo (timeseriesData) {
  // TimeseriesDataArray is an array of arrays with timeseries for each voi.
  const timeseriesDataArray = timeseriesData[0]
  function helper (timeseries, result) {
    /* Iterate columnwise
      FROM:
      [[voi1_timeseries_el1, voi1_timeseries_el2],
  ...
  return helper(timeseriesDataArray[0].data, [])
}
...
function convertUnitsForCosmo (voiConfigs, timeseriesDataCollection, parameter) {
  // For each variable of interest, return the timeseries with converted values
  const data = {}
  _.forEach(voiConfigs, (voiConfig) => {
    const keyBeob = voiConfig[parameter].key
...
async function convertUnitsForCosmo (voiConfigs, timeseriesDataCollection, parameter) {
  // For each variable of interest, return the timeseries with converted values
  const data = {}
  _.forEach(voiConfigs, (voiConfig) => {
    const keyBeob = voiConfig.cosmo.options.key
    const keyTarget = voiConfig.target.key
    _.forEach(timeseriesDataCollection.data, (timeseriesDataItem) => {
      data[keyTarget] = _.map(timeseriesDataItem[keyBeob], (item) => {
        return {
          timestamp: item.timestamp,
          value: convertUnit(
  ...
  const keyBeob = voiConfigs[0].cosmo.options.key
  const keyTarget = voiConfigs[0].target.unit
  //return [voiConfigs, timeseriesDataCollection, keyBeob, keyTarget]
...
function renderMeasuredValuesAsCSVMosmix (voiConfigs, timeseriesDataArray) {
  const csvLabels = getCSVLabels(voiConfigs, timeseriesDataArray)
  const csvStrings = getStringsFromStationDataPayloadMosmix(timeseriesDataArray)
  return csvLabels + csvStrings.join('\n')
}

function renderMeasuredValuesAsCSVCosmo (voiConfigs, timeseriesDataArray) {
  const csvLabels = getCSVLabels(voiConfigs, timeseriesDataArray)
  const csvStrings = getStringsFromStationDataPayloadCosmo(timeseriesDataArray)
  return csvLabels + csvStrings.join('\n')
}
...
exports.getStringsFromStationDataPayloadMosmix = getStringsFromStationDataPayloadMosmix
exports.getStringsFromStationDataPayloadCosmo = getStringsFromStationDataPayloadCosmo
...
exports.renderMeasuredValuesAsCSVMosmix = renderMeasuredValuesAsCSVMosmix
exports.renderMeasuredValuesAsCSVCosmo = renderMeasuredValuesAsCSVCosmo
