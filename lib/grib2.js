...
log.info('loaded module used by poi_forecast_engine for dealing with grib2-files')

function getGribDirectoryPath (startUnixEpoch, voi) {
  const startMoment = moment.unix(startUnixEpoch)
  let startHour = '' + Math.floor(startMoment.hour() / 3) * 3

  let month = '' + (startMoment.month() + 1)
  if (month.length < 2) {
    month = '0' + month
  }

  let day = '' + startMoment.date()
  if (day.length < 2) {
    day = '0' + day
  }
  ...
  const pathElements = ['' + startMoment.year() + month + day + startHour, voi]
...
async function readValuesFromGrib2BzFile (pathToFile, position) {
  ...
  if (process.platform === 'win32') {
    await exec(path.join('bin', '7z.exe') + ' -o' + tmpDir.path + ' e ' + pathToFile, {
      maxBuffer: 10 * 1024 * 1024
    })
  } else {
    try {
      await exec(
        'lz4 -d ' + pathToFile + ' ' + path.join(tmpDir.path, 'dummy_file_name.grib2')
      )
    } catch (error) {
  ...
  const resultOfReadDir = await fs.readdir(tmpDir.path)
...
async function readTimeSeriesFromGribFiles (
  ...
  const gribDirectoryPath = path.join(
    gribBasePath,
