...
function getValuesAsCSVString (timeStampsAndValues) {
  ...
  _.forEach(timeStampsAndValues, function (item) {
    const value = item ? item.value : NULL_VALUE
...
function renderTimeseriesAsCSVMosmix (voiConfigs, timeseriesDataArray) {
...
function renderTimeseriesAsCSVCosmo (voiConfigs, timeseriesDataArray) {
  const csvLabels = getCSVLabels(voiConfigs, timeseriesDataArray)
  let formatted = {}
  _.forEach(timeseriesDataArray, function (element) {
    let label = element.label
    let data = element.data
    formatted[label] = data
  } )
  const csvStrings = getStringsFromStationDataPayload(formatted)
  return csvLabels + csvStrings.join('\n')
}
// Export functions
exports.renderTimeseriesAsCSVMosmix = renderTimeseriesAsCSVMosmix
exports.renderTimeseriesAsCSVCosmo = renderTimeseriesAsCSVCosmo
