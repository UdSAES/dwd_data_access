...
function getStationById (stations, stationId) {
  return stations.filter((station) => station.stationId === stationId)[0]
}
...
function findStationsInRadiusAroundCoordinates (stations, coordinates, radius, limit) {
...
function shortenTimeSeriesToPeriodCosmo (timeseries, from, to) {
  ...
  for (let i = 0; i < timeseries.length; i += 1) {
    ...
    const data = timeseries[i].data.filter(el => (el.timestamp >= from && el.timestamp <= to))
