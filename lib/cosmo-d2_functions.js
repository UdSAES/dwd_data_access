...
const c2dh = require('./cosmo-d2_helper')
...
async function loadGribTimeseriesData (options) {
  ...
  const directoryPath = c2dh.deriveGrib2DirectoryPath(
    gribBaseDirectory,
    referenceTimestamp,
    voi
  )
  const timeseriesData = await c2dh.extractTimeseriesDataFromGrib2Directory(
    directoryPath,
    location
  )
