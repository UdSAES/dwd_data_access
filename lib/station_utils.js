...
function sortStationsByDistanceApplyLimit (
  stations,
  coordinates,
  radius,
  limit
...
function findStationsInRadiusAroundCoordinates (
  stations,
  coordinates,
  radius,
  limit
  ...
  _.forEach(stations, function (stationEntry) {
    const stationLocation = getStationLocation(stationEntry)
    if (
      geolib.isPointWithinRadius(stationLocation, coordinates, radiusInMeters)
    ) {
      const dist = geolib.getDistance(coordinates, stationLocation)
      const stationObj = { station: stationEntry, distance: inKilometers(dist) }
...
function sortStationsByDistanceApplyRadius (
  stations,
  coordinates,
  radius,
  limit
) {
  return findStationsInRadiusAroundCoordinates(stations, coordinates, radius)
}

function sortStationsByDistanceApplyRadiusAndLimit (
  stations,
  coordinates,
  radius,
  limit
) {
  const stationsList = sortStationsByDistanceApplyRadius(
    stations,
    coordinates,
    radius
  )
...
// Declaration of a controller: which function to use according to the arguments passed
const dispatcher = new ArrayKeyedMap() // for mapping [vicinity, radius, limit] to function
dispatcher.set(
...
dispatcher.set(
  ...
  () => findStationsAroundCoordinates
)
dispatcher.set(
  ['object', 'undefined', 'number'],
  () => sortStationsByDistanceApplyLimit
)
dispatcher.set(
  ['object', 'number', 'undefined'],
  () => sortStationsByDistanceApplyRadius
)
dispatcher.set(
  ['object', 'number', 'number'],
  () => sortStationsByDistanceApplyRadiusAndLimit
)
