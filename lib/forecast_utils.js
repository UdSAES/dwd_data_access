...
function renderForecastCSV (voiConfigs, timeseriesDataArray) {
...
async function loadCosmoData (voiConfig, referenceTimestamp, latitude, longitude, gribBaseDirectory) {
...
function readDataCosmoD2 (
  ...
  const latitude = stationLocation.latitude
  const longitude = stationLocation.longitude

  // Works with old and new directory structure.
  // const directories = getCosmoDirectories(WEATHER_DATA_BASE_PATH, startTimestamp, endTimestamp, modelRun)
  ...
  while (closestValidTimestamp <= endTimestamp) {
    ...
    _.forEach(voiConfigs, async (voiConfig) => {
      const data = loadCosmoData(voiConfig, closestValidTimestamp, latitude, longitude, gribBaseDirectory)
...
exports.renderForecastAsJSON = renderForecastAsJSON
exports.readMosmixTimeseriesData = readMosmixTimeseriesData
exports.renderForecastCSV = renderForecastCSV
