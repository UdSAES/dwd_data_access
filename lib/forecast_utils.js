...
const gh = require('dwd-grib-helper')
...
function cosmoRenderJson (
  ...
  _.forEach(voiConfigs, (voiConfig) => {
    result.data.push({
      label: voiConfig.target.key,
      unit: voiConfig.target.unit,
      timeseries: timeseriesDataArray[0].data
    })
  })
...
function renderForecastCSVMosmix (voiConfigs, timeseriesDataArray) {
  return gu.renderMeasuredValuesAsCSVMosmix(voiConfigs, timeseriesDataArray)
}

function renderForecastCSVCosmo (voiConfigs, timeseriesDataArray) {
  return gu.renderMeasuredValuesAsCSVCosmo(voiConfigs, timeseriesDataArray)
}
...
function getConfigFromModel (model, COSMO_DATA_BASE_PATH, MOSMIX_DATA_BASE_PATH) {
  if (model === 'mosmix') {
    return {
      ...
      jsonRenderer: mosmixRenderJson,
      csvRenderer: renderForecastCSVMosmix,
    }
    ...
    return {
      ...
      unitsConverter: (a, value, b) => value,//gu.convertUnitsForCosmo,
      jsonRenderer: cosmoRenderJson,
      csvRenderer: renderForecastCSVCosmo
    }
  } else {
    return {}
  }
...
function getCosmoDirectories (WEATHER_DATA_BASE_PATH, startTimestamp, endTimestamp, modelRun) {
  let timestamp = startTimestamp
  let gribBaseDirectory
  const directories = []
  while (timestamp <= endTimestamp) {
    gribBaseDirectory = getGribDirectory(timestamp, WEATHER_DATA_BASE_PATH)
    const dayDateTimeString =
    moment
      .utc(timestamp)
      .format('YYYYMMDD') + modelRun
    const directory = gribBaseDirectory + "/" + dayDateTimeString
    timestamp += hoursInDay * secInHour * milliseconds
...
function findClosestValidTimestamp(timestamp, modelRun) {
  ...
  if (moment(dayDateTimeString).isAfter(moment(closestModelRunTimestampSameDate))) {
    ...
    const validTimestamp = parseInt(moment.utc(closestModelRunTimestampSameDate).add(1, 'day').format('x'))
    const date = moment.utc(validTimestamp).format('YYYYMMDDTHH')
    //return [closestModelRunTimestampSameDate, timestamp, validTimestamp, date]
    ...
    const validTimestamp = parseInt(moment.utc(closestModelRunTimestampSameDate).format('x'))
    const date = moment.utc(validTimestamp).format('YYYYMMDDTHH')
    //return [closestModelRunTimestampSameDate, timestamp, validTimestamp, date]
    return validTimestamp
  }
...
async function loadCosmoData (voiConfig, referenceTimestamp, lat, lon, gribBaseDirectory) {
...
async function asyncIterator (voisConfigs) {
  const timeseries = []
  for (let i = 0; i < voisConfigs.length; i += 1) {
    try {
      const data = await loadCosmoData(voisConfigs[i], referenceTimestamp, lat, lon, gribBaseDirectory)
...
async function readDataCosmoD2 (
  ...
  while (closestValidTimestamp <= endTimestamp) {
    const gribBaseDirectory = getGribDirectory(closestValidTimestamp, WEATHER_DATA_BASE_PATH)
    for (let i = 0; i < voisConfigs.length; i += 1) {
      try {
        const data = await loadCosmoData(voisConfigs[i], closestValidTimestamp, lat, lon, gribBaseDirectory)
        timeseries.push(data)
      } catch (error) {
        log.warn(error)
      }
...
exports.mosmixRenderJson = mosmixRenderJson
exports.cosmoRenderJson = cosmoRenderJson
exports.readMosmixTimeseriesData = readMosmixTimeseriesData
exports.renderForecastCSVMosmix = renderForecastCSVMosmix
exports.renderForecastCSVCosmo = renderForecastCSVCosmo
