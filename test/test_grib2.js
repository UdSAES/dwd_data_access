...
const TEST_DATA_URL =
  'http://static_file_server.agnew.msaas.me/test_data/dwd_forecast_service/sample_data.zip'
const GRIB_TEST_FILE_PATH =
  './2018011803/t_2m/COSMODE_single_level_elements_T_2M_2018011803_000.grib2.bz2'
...
describe.skip('./lib/grib2.js', () => {
  ...
  describe('readTimeSeriesFromGribFiles()', () => {
    it('should read a time series from multiple grib files', async () => {
      const result = await grib2.readTimeSeriesFromGribFiles(
        VALID_GRIB_START_TIMESTAMP,
        { longitude: 7, latitude: 49 },
        't_2m',
        path.join(tmpDirPath, 'sample_data', 'grib')
      )
    ...
    it('should return the error NO_DATA_AVAILABLE, if no data is available', async () => {
      try {
        await grib2.readTimeSeriesFromGribFiles(
          0,
          { longitude: 7, latitude: 49 },
          't_2m',
          path.join(tmpDirPath, 'sample_data', 'grib')
  ...
  describe('readValueFromGrib2BzFile()', () => {
    it('should return the value from a grib file for the given position', async () => {
      const result = await grib2.readValuesFromGrib2BzFile(
    ...
    it('should return an error when the file does not exist', async () => {
      try {
        await grib2.readValueFromGrib2BzFile(
          path.join(tmpDirPath, 'sample_data', 'grib', 'NONE_SENSE'),
          GRIB_TEST_POSITION
        )
      } catch (error) {
        assert(_.isError(error))
